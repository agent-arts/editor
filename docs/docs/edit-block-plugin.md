...
![](../public/edit-block-plugin-1.png)
...
该接口定义了 `EditBlockWidget` 委托回宿主层的三个操作。它是插件的 DOM 渲染与应用的块管理逻辑之间的**唯一耦合点**。
...
在**粘贴操作**期间，`addBlockAtEffect` 尤为重要：当用户粘贴包含 `{#EditorBlock#}` 标记的序列化内容时，粘贴处理程序会解析它们，分发文档变更，然后为每个解析出的块在其正确的偏移位置触发此 Effect。
...
![](../public/edit-block-plugin-2.png)
...
`update()` 方法处理两类变更：**文档变更**和 **Effect 分发**。
...
`CustomEditor` 类是 EditBlock 插件的主要消费者，充当它与编辑器系统其余部分之间的桥梁。该集成涉及四个方面：初始化、块 CRUD、序列化和粘贴处理。
...
该接口不被插件的内部机制使用——它作为包装编辑器的框架集成（Vue、Angular）的**推荐契约**，为弹出框协调提供类型化的结构。
